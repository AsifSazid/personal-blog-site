<x-backend.layouts.master>
    <x-slot name="header">
        <div class="flex items-center justify-between px-4 py-4 border-b lg:py-6 dark:border-primary-darker">
            <h2 class="text-2xl font-semibold">{{ __('Galleries') }}</h2>
            <a href="{{ route('admin.galleries.create') }}" class="text-green-500 hover:text-green-700 mx-1" title="Create">
    ...
    <div class="overflow-x-auto">
        ...
        <table id="roleTable" class="w-full table-striped table-bordered text-sm mt-4">
            <thead class="bg-gray-100 text-gray-700 uppercase">
                <tr>
                    ...
                    <th class="px-6 py-4">Gallery Name</th>
        ...
        <div class="mt-4 px-6">
            ...
            <div class="px-4 py-2 bg-gray-100 border-t text-sm text-gray-500 flex justify-between items-center">
                <a href="{{ route('admin.galleries.trash') }}" class="text-red-500 hover:text-red-700 mx-1" title="Trash Lists">
    ...
    @push('js')
        <script>
            document.addEventListener("DOMContentLoaded", () => {
                ...
                const fetchRoles = async (search = '', page = 1) => {
                    try {
                        const response = await fetch(
                            `{{ route('admin.galleries.getData') }}?search=${search}&page=${page}`, {
                ...
                const renderTable = (galleries) => {
                    tableBody.innerHTML = "";
                    if (!galleries || galleries.length === 0) {
                        tableBody.innerHTML =
                            `<tr><td colspan="5" class="py-4 text-center text-gray-500">No galleries found</td></tr>`;
                        return;
                    }
                    ...
                    galleries.forEach((gallery, index) => {
                        const row = `
                                <tr>
                                    ...
                                    <td class="px-6 py-4">${gallery.remarks ? gallery.remarks : 'No Data Found'}</td>
                                    <td class="px-6 py-4">
                                        <div class="flex justify-center">
                                            <a href="/admin/galleries/${gallery.uuid}" class="px-1 text-blue-500 hover:text-blue-700" title="View"><i class="fas fa-eye"></i></a>
                                            <a href="/admin/galleries/${gallery.uuid}/edit" class="px-1 text-yellow-500 hover:text-yellow-700" title="Edit"><i class="fas fa-edit"></i></a>
                                            <form action="/admin/galleries/${gallery.uuid}" method="POST" onsubmit="return confirm('Move to trash?')">
