<x-backend.layouts.master>
    <x-slot name="header">
        <div class="flex items-center justify-between px-4 py-4 border-b lg:py-6 dark:border-primary-darker">
            ...
            <a href="{{ route('categories.create') }}" class="text-green-500 hover:text-green-700 mx-1" title="Create">
    ...
    <div class="overflow-x-auto">
        ...
        <div class="mt-4 px-6">
            ...
            <div class="px-4 py-2 bg-gray-100 border-t text-sm text-gray-500 flex justify-between items-center">
                <a href="{{ route('categories.trash') }}" class="text-red-500 hover:text-red-700 mx-1" title="Trash Lists">
                ...
                const fetchRoles = async (search = '', page = 1) => {
                    try {
                        const response = await fetch(
                            `{{ route('categories.getData') }}?search=${search}&page=${page}`, {
                                ...
                                <tr>
                                    ...
                                    <td class="px-6 py-4">
                                        <div class="flex justify-center">
                                            <a href="/categories/${category.uuid}" class="px-1 text-blue-500 hover:text-blue-700" title="View"><i class="fas fa-eye"></i></a>
                                            <a href="/categories/${category.uuid}/edit" class="px-1 text-yellow-500 hover:text-yellow-700" title="Edit"><i class="fas fa-edit"></i></a>
                                            <form action="/categories/${category.uuid}" method="POST" onsubmit="return confirm('Move to trash?')">
