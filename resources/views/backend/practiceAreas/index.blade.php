<x-backend.layouts.master>
    <x-slot name="header">
        <div class="flex items-center justify-between px-4 py-4 border-b lg:py-6 dark:border-primary-darker">
            <h2 class="text-2xl font-semibold">{{ __('PracticeAreas') }}</h2>
            <a href="{{ route('admin.practice_areas.create') }}" class="text-green-500 hover:text-green-700 mx-1" title="Create">
    ...
    <div class="overflow-x-auto">
        ...
        <table id="roleTable" class="w-full table-striped table-bordered text-sm mt-4">
            <thead class="bg-gray-100 text-gray-700 uppercase">
                <tr>
                    ...
                    <th class="px-6 py-4">Practice Area Title</th>
        ...
        <div class="mt-4 px-6">
            ...
            <div class="px-4 py-2 bg-gray-100 border-t text-sm text-gray-500 flex justify-between items-center">
                <a href="{{ route('admin.practice_areas.trash') }}" class="text-red-500 hover:text-red-700 mx-1" title="Trash Lists">
                ...
                const fetchRoles = async (search = '', page = 1) => {
                    try {
                        const response = await fetch(
                            `{{ route('admin.practice_areas.getData') }}?search=${search}&page=${page}`, {
                        ...
                        console.error("Error fetching practiceAreas:", error);
                    }
                };

                const renderTable = (practiceAreas) => {
                    tableBody.innerHTML = "";
                    if (!practiceAreas || practiceAreas.length === 0) {
                        tableBody.innerHTML =
                            `<tr><td colspan="5" class="py-4 text-center text-gray-500">No Practice Areas found</td></tr>`;
                        return;
                    }

                    practiceAreas.forEach((practiceArea, index) => {
                        const row = `
                                <tr>
                                    <td class="px-6 py-4">${index + 1}</td>
                                    <td class="px-6 py-4">${practiceArea.title}</td>
                                    <td class="px-6 py-4">${practiceArea.remarks ? practiceArea.remarks : 'No Data Found'}</td>
                                    <td class="px-6 py-4">
                                        <div class="flex justify-center">
                                            <a href="/admin/practice_areas/${practiceArea.uuid}" class="px-1 text-blue-500 hover:text-blue-700" title="View"><i class="fas fa-eye"></i></a>
                                            <a href="/admin/practice_areas/${practiceArea.uuid}/edit" class="px-1 text-yellow-500 hover:text-yellow-700" title="Edit"><i class="fas fa-edit"></i></a>
                                            <form action="/admin/practice_areas/${practiceArea.uuid}" method="POST" onsubmit="return confirm('Move to trash?')">
