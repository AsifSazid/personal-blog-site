<x-backend.layouts.master>
    <x-slot name="header">
        <div class="flex items-center justify-between px-4 py-4 border-b lg:py-6 dark:border-primary-darker">
            <h2 class="text-2xl font-semibold">{{ __('Blogs') }}</h2>
            <a href="{{ route('admin.blogs.create') }}" class="text-green-500 hover:text-green-700 mx-1" title="Create">
    ...
    <div class="overflow-x-auto">
        ...
        <table id="roleTable" class="w-full table-striped table-bordered text-sm mt-4">
            <thead class="bg-gray-100 text-gray-700 uppercase">
                <tr>
                    ...
                    <th class="px-6 py-4">Blog Name</th>
        ...
        <div class="mt-4 px-6">
            ...
            <div class="px-4 py-2 bg-gray-100 border-t text-sm text-gray-500 flex justify-between items-center">
                <a href="{{ route('admin.blogs.trash') }}" class="text-red-500 hover:text-red-700 mx-1" title="Trash Lists">
            ...
            document.addEventListener("DOMContentLoaded", () => {
                ...
                const renderTable = (blogs) => {
                    ...
                    if (!blogs || blogs.length === 0) {
                        tableBody.innerHTML =
                            `<tr><td colspan="5" class="py-4 text-center text-gray-500">No blogs found</td></tr>`;
                        return;
                    }
                    ...
                    blogs.forEach((blog, index) => {
                        const row = `
                                <tr>
                                    ...
                                    <td class="px-6 py-4">${blog.remarks ? blog.remarks : 'No Data Found'}</td>
                                    <td class="px-6 py-4">
                                        <div class="flex justify-center">
                                            <a href="/admin/blogs/${blog.uuid}" class="px-1 text-blue-500 hover:text-blue-700" title="View"><i class="fas fa-eye"></i></a>
                                            <a href="/admin/blogs/${blog.uuid}/edit" class="px-1 text-yellow-500 hover:text-yellow-700" title="Edit"><i class="fas fa-edit"></i></a>
                                            <form action="/admin/blogs/${blog.uuid}" method="POST" onsubmit="return confirm('Move to trash?')">
